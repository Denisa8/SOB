...
namespace TorrentClient
{
  class Program
  {
    ...
    public static readonly int trackerPort = 44389;
    ...
    static async Task Main(string[] args)
    {
      ...
      if (torrentsPath.EndsWith(".torrent", StringComparison.OrdinalIgnoreCase))
      {
        try
        {
          ...
          #endregion
          #region Stworzenie peera
          peer = new Peer(torrentFileInfo);
          //peer.EnablePeerConnections(port); //nasłuchujemy zgłoszeń od peerów
          #endregion
          #region Tracker
          ...
          InitConnectToTracker connSettings = new InitConnectToTracker(ID, Peer.peerIp, port, Files.ToDictionary(x => x.Key, x => x.Value), bannedPeers);
        ...
        if (ob != null)
        {
          ...
          else if (ob.Type == typeof(List<ConnectedPeer>))
          {
            ...
            foreach (var l in lcp)
            {
              ...
              peer.ConnectToPeer(l.IP, l.Port);
          ...
          else if (ob.Type == typeof(ConnectedPeer))
          {
            ...
            peer.ConnectToPeer(cp.IP, cp.Port);
          }
